...
{
  enum intr_level old_level = intr_disable ();
  int pri_value = thread_current() -> priority;
  intr_set_level (old_level);
  return pri_value;
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  // initializations for priority donation
  t->init_priority = priority;
  t->wait_on_lock = NULL;
  list_init(&t->donating_threads);

  // initialized for BSD scheduler
  t->nice = 0;
  t->recent_cpu = 0;
...
bool priority_comparator(const struct list_elem *a,const struct list_elem *b,void *aux UNUSED){
  ...
  return false; //else if(ta -> priority <= tb -> priority) 

}


bool ticks_comparator(const struct list_elem *a,const struct list_elem *b,void *aux UNUSED){

  struct thread *ta = list_entry(a,struct thread,elem);
  struct thread *tb = list_entry(b,struct thread,elem);

  if(ta -> wakeupTime < tb -> wakeupTime)
    return true;
...
void remove_with_lock(struct lock *lock){
  ...
  struct list_elem *e = list_begin(&thread_current()->donating_threads);
  struct list_elem *next;
  
  while (e != list_end(&thread_current()->donating_threads)){
...
void refresh_priority (void){
  ...
  if (list_empty(&t->donating_threads))
      return;
  
  struct thread *s = list_entry(list_front(&t->donating_threads),
...
void check_highest_priority (void){
 ...
 void mlfqs_priority (struct thread *t){
  ...
  int t1 = int_to_fp(PRI_MAX);
  int t2 = div_mixed( t->recent_cpu, 4);
  int t3 = 2*t->nice;
  
  t1 = sub_fp(t1, t2);
  t1 = sub_mixed(t1, t3);
  t->priority = fp_to_int(t1);
...
void mlfqs_recent_cpu (struct thread *t){
  ...
  t1 = div_fp(t1, add_mixed(t1, 1) );
  t1 = mult_fp(t1, t->recent_cpu);
  t->recent_cpu = add_mixed(t1, t->nice);
}


void mlfqs_load_avg (void){
  int t2 = list_size(&ready_list);
  if (thread_current() != idle_thread){
      t2++;
    }
  int t1 = div_mixed(int_to_fp(59), 60);
  t1 = mult_fp(t1, load_avg);
  t2 = div_mixed(int_to_fp(t2), 60);
  load_avg = add_fp(t1, t2);
  ASSERT (load_avg >= 0);
}
